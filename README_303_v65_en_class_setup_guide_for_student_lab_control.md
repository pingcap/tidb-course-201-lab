...
2. On the day of training, download private key file (`*.pem`), and then set the permission is `400`.
   ```
   $ mv <key_file> ~/.ssh/ 
...
2. On the day of training, convert the private key file (`*.pem`), to `*.ppk` format using [PuTTYgen](https://www.puttygen.com/). 
