# B2: Python 批量插入（输出）
+ 以下输出示例来自 TiUP 游乐场
+ 如果客户端和 TiDB Cloud 不在同一个区域,则两次执行之间的 `elapsed time` 差距会很大
  + 如果你迫不及待地等待第二个非批量模式运行完成,请随时使用 `ctrl-c` 退出
```8
...
如果客户端和 TiDB Cloud 不在同一个区域,以我的极端例子为例,我在 us-west-2 中对照 TiDB Cloud 在上海的一个节点上运行了演示脚本,则引发了 DatabaseError 8229:
