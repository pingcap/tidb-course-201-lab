...
def _insert_new_planet(cursor, ps_stmt, name, mass, cat_id) -> int:
    cursor.execute(ps_stmt, (name, mass, 1, cat_id, datetime.datetime.now()))
        ...
        """
            while True:
                ...
                try:  # Cursor execute DML scope
                    ...
                    last_gen_id = _insert_new_planet(
                        cursor,
                        ps_insert,
                        new_planet_name,
                        new_planet_mass,
                        new_planet_category_id,
                    )
                    print(
                        "Inserting new planet: Id("
                        + str(last_gen_id)
                        + "), Name("
                        + new_planet_name
                        + "), Mass("
