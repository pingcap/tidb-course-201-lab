...
public class DemoJdbcMyDBWorkload {
    public static void main(String[] args) {
        Connection connection = null;
        String username = "mydb_app";
        try {
            connection = DriverManager.getConnection(
                    "jdbc:mysql://localhost:4000/mydb", username, "tidb");
            Statement statement = connection.createStatement();
            int rowCount = 0;
            long startTime = 0L;
            for (int i = 0; i < 50; i++) {
                startTime = System.currentTimeMillis();
                rowCount = statement.executeUpdate("UPDATE payments SET amount = " + i * 70);
