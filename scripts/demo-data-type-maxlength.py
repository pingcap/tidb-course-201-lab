...
from mysql.connector.errors import ProgrammingError as perr
...
def _setup(try_char_def, try_varchar_def):
  ...
  while True:
    try:
      ...
      ps_create_table = dyc_creation_ddl_stmt.format(try_char_def, try_varchar_def)
  ...
  while True:
    try:
      cursor.execute(ps_drop_table)
      ps_create_table = dyc_creation_ddl_stmt.format(max_char_def, try_varchar_def)
      cursor.execute(ps_create_table)
      max_varchar_def = try_varchar_def
      try_varchar_def += 1
