...
/* Populate seed */
INSERT INTO test.auto_increment_t1_clustered (name, varname)
VALUES ('A', 'V1'),
    ('B', 'V1'),
    ('C', 'V2'),
    ('D', 'V2');
/* Populate dummy data: Flooding with data 2M rows to t1 */
INSERT INTO test.auto_increment_t1_clustered
SELECT null,
    name,
    varname
...
SELECT *
FROM (
        SELECT COUNT(region_id) AS "Clustered # of TiKV Regions"
        FROM information_schema.tikv_region_status
        WHERE table_name = 'auto_increment_t1_clustered'
    ) v1
    join (
        SELECT COUNT(region_id) AS "Non-Clustered # of TiKV Regions"
        FROM information_schema.tikv_region_status
        WHERE table_name = 't2_nonclustered'
    ) v2;
