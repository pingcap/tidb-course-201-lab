...
ssh ${HOST_KV1_PRIVATE_IP} unzip -u stage/tidb-admin-dataset.zip -d stage/
...
ssh ${HOST_KV1_PRIVATE_IP} mkdir /tmp/backup
ssh ${HOST_KV2_PRIVATE_IP} mkdir /tmp/backup
ssh ${HOST_KV3_PRIVATE_IP} mkdir /tmp/backup
