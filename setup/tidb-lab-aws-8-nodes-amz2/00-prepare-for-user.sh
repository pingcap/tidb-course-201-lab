...
--filter "Name=instance-state-name,Values=running" "Name=tag:class-${USER},Values=kv3" \
--query "Reservations[0].Instances[0].PrivateIpAddress" \
--output text \
--region ${REGION_CODE}`
...
--filter "Name=instance-state-name,Values=running" "Name=tag:class-${USER},Values=db1" \
--query "Reservations[0].Instances[0].PrivateIpAddress" \
--output text \
--region ${REGION_CODE}`
...
sed -i '' \
  ...
  -e "s/<HOST_DB2_PRIVATE_IP>/${HOST_DB1_PRIVATE_IP}/g" \
